...
export default function App() {
  ...
  // Max possible fine = what you'd owe if you did NOTHING this week
  const calcMaxGoalFine = useCallback((goal) => {
    const cat = GOAL_CATEGORIES.find(c => c.id === goal.category);
    const fine = goal.fineAmount || 0;
    const target = goal.weeklyTarget || 0;
    if (goal.category === "junk") return 0; // junk has no max (unlimited overage)
    if (cat?.logType === "tick" || (goal.logType || cat?.logType) === "tick") return target * fine;
    return fine; // one fine for missing weekly numeric target
  }, []);

  const calcMaxMemberFine = useCallback((memberId) => {
    return goals.filter(g => g.memberId === memberId && g.active !== false)
      .reduce((sum, g) => sum + calcMaxGoalFine(g), 0);
  }, [goals, calcMaxGoalFine]);

  // Fine reduced by day logs — how much earned back so far
  const calcGoalFineFromDayLogs = useCallback((goal, memberId, wk) => {
    const override = getFine(memberId, goal.id, wk);
    if (override?.overridden) return override.customAmount != null ? override.customAmount : 0;
    const weekDts = getWeekDates(wk);
    const cat = GOAL_CATEGORIES.find(c => c.id === goal.category);
    const fine = goal.fineAmount || 0;
    const target = goal.weeklyTarget || 0;
    const isTickType = (goal.logType || cat?.logType) === "tick";

    if (goal.category === "junk") {
      // Count total junk meals logged this week
      const totalJunk = weekDts.reduce((s, d) => {
        const dl = dayLogs[`${memberId}__${goal.id}__${d}`];
        return s + (dl?.value || 0);
      }, 0);
      return Math.max(0, totalJunk - target) * fine;
    }

    if (isTickType) {
      const doneDays = weekDts.filter(d => dayLogs[`${memberId}__${goal.id}__${d}`]?.done).length;
      const missed = Math.max(0, target - doneDays);
      return missed * fine;
    }

    // Numeric weekly — use weekLog value
    const log = getLog(memberId, goal.id, wk);
    if (!log) return fine; // not logged = full fine
    return (log.value || 0) >= target ? 0 : fine;
  }, [goals, dayLogs, weekLogs, fines]);

  const calcMemberWeekFine = useCallback((memberId, wk) => {
    const mGoals = goals.filter(g => g.memberId === memberId && g.active !== false);
    return mGoals.reduce((sum, g) => sum + calcGoalFineFromDayLogs(g, memberId, wk), 0);
  }, [goals, calcGoalFineFromDayLogs]);
  ...
  const getMemberWeekFineDetail = useCallback((memberId, wk) => {
    ...
    return mGoals.map(g => {
      ...
      const maxFine = calcMaxGoalFine(g);
      const currentFine = calcGoalFineFromDayLogs(g, memberId, wk);
      const rawFine = calcGoalFine(g, log);
      const fine = override?.overridden
        ? (override.customAmount != null ? override.customAmount : 0)
        : currentFine;
      return { goal: g, log, fine, override, rawFine, maxFine, currentFine };
    });
  }, [goals, weekLogs, dayLogs, fines, calcMaxGoalFine, calcGoalFineFromDayLogs]);
